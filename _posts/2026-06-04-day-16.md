...
```
[ Git Event Trigger ] ──► [ Build Neutral Image ] ──► [ Push to Docker Hub ]│┌─────────────────────────┼─────────────────────────┐▼                         ▼                         ▼(Deploy to Dev)          (Deploy to Stage)          (Deploy to Prod)Inject: Mint Green       Inject: Amber Orange       Inject: Slate Blue
```
...
```
[ develop ] ──► Create Branch ──► [ release/ ] ──► Deploys to Staging (Orange)│▼ (QA Passes)Raise PR to [ main ]│▼ (Triggers Pre-Merge Prod Deployment)[ Production Gate ]  ◄── (Manual Sign-Off)│▼ (Approved)[ Live Production ] ──► Auto-Merges to main & develop
```
